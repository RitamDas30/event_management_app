...
const statusConfig = {
  registered: { label: "Confirmed", color: "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10", icon: CheckCircle },
  waitlisted: { label: "Waitlisted", color: "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10", icon: Clock },
  cancelled: { label: "Cancelled", color: "text-rose-600 dark:text-rose-400 bg-red-50", icon: XCircle },
};
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-surface-600 dark:text-surface-400 mt-1">Your registrations and digital tickets</p>
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id
                  ? "bg-surface-50 dark:bg-surface-900 text-brand-600 dark:text-brand-400 shadow-sm"
                  : "text-surface-600 dark:text-surface-400 hover:text-surface-950 dark:text-surface-50"
              }`}
            >
              <Icon className="w-4 h-4" />
              {tab.label}
              {tab.id === "tickets" && (
                <span className="ml-1 text-xs bg-brand-100 dark:bg-brand-500/20 text-brand-600 dark:text-brand-400 px-1.5 py-0.5 rounded-full">
      ...
      {loading ? (
        ...
        activeTab === "tickets" ? (
            ...
            {filtered.map((reg) => (
              <div key={reg._id} className="bg-surface-50 dark:bg-surface-900 rounded-xl border border-border overflow-hidden">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 text-white">
                  ...
                  <span className="text-xs bg-surface-50 dark:bg-surface-900/20 px-2 py-0.5 rounded-full ml-2">{reg.event.category}</span>
                </div>
                <div className="p-4">
                  ...
                  {reg.qrCode ? (
                    <div className="flex flex-col items-center py-4 border-t border-dashed border-border">
                      ...
                      <p className="text-xs text-surface-500 mb-2">Scan at entrance</p>
                      <button onClick={() => downloadQR(reg.qrCode, reg.event.title)} className="flex items-center gap-1 text-sm text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:text-brand-300 font-medium">
                        <Download className="w-4 h-4" /> Download
                      </button>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center py-6 border-t border-dashed border-border">
                      <QrCode className="w-10 h-10 text-surface-300 dark:text-surface-700 mb-2" />
                      <p className="text-sm text-surface-500">QR not available</p>
                    </div>
                  )}
          ...
          <div className="space-y-3">
            {filtered.map((reg) => {
              ...
              return (
                <div key={reg._id} className="bg-surface-50 dark:bg-surface-900 rounded-xl border border-border p-4 flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <Link to={getEventPath(reg.event._id)} className="text-base font-semibold text-surface-950 dark:text-surface-50 hover:text-brand-600 dark:text-brand-400 transition-colors">
                      {reg.event.title}
                    </Link>
                  ...
                  <div className="flex items-center gap-3">
                    {reg.qrCode && reg.status === "registered" && (
                      <img src={reg.qrCode} alt="QR" className="w-16 h-16 rounded border border-border" />
                    )}
        ...
        <div className="text-center py-20">
          ...
          <p className="text-surface-500 mt-2">Browse events and register to see them here</p>
          <Link to={user?.role === "student" ? "/student/explore" : "/explore"} className="mt-4 inline-block text-sm font-medium text-brand-600 dark:text-brand-400">
            Explore Events
          </Link>
        </div>
      )}
