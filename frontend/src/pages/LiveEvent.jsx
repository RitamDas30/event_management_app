...
import { ArrowLeft, Wifi, Users, Send, MessageSquare } from "lucide-react";
...
export default function LiveEvent() {
  ...
  const startJitsi = useCallback(() => {
    ...
    const isOrg = event.organizer?._id === user?.id || event.organizer === user?.id;
    ...
    try {
      ...
      // Force the iframe to fill container
  ...
  useEffect(() => {
    ...
    const loadAndStart = () => {
      ...
      script.async = true;
      script.onload = () => {
        // Small delay to ensure API is fully available
        setTimeout(startJitsi, 300);
      };
      document.head.appendChild(script);
    };

    // Wait for container to be in DOM with dimensions
